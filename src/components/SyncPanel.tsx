...
import { CloudOff, RefreshCw, UsersRound, UploadCloud } from "lucide-react";
import db, { getSyncStatus, seedSampleChildren, syncWithServer } from "@/lib/db";
...
export function SyncPanel() {
  ...
  const handleSeedSamples = async () => {
    const result = await seedSampleChildren();
    if (result.added > 0) {
      setMessage(`${result.added} enfants test ajoutés localement.`);
    }
  };
    ...
    <section className="mt-6 rounded-lg border border-gray-200 bg-white p-4 shadow-sm">
      ...
      {!isLoading && childrenCount === 0 && (
        <button
          ...
          className="mt-3 flex h-11 w-full items-center justify-center gap-2 rounded-lg border border-dashed border-[#00b22d] text-sm font-bold text-[#00b22d]"
        >
          <UsersRound className="h-5 w-5" />
          Ajouter enfants test
        </button>
      )}
